...
> ⚠️ **注意**：秘密房间中的 ARG《第五张财签》是一个**独立项目，不在本仓库内**，详见文末[「相关项目」](#-相关项目)。
...
秘密房间（`/who_i_am/secret_quiz/secret_room`）里有一个「ARG」入口按钮，点击后在弹窗中显示「第五张财签」卡片，卡片指向 `/arg_01/`。

**该 ARG 是一个完全独立的项目：源码、仓库、构建、部署全部与本仓库无关，不在本仓库内维护或存放。** 本仓库只包含一个指向它的入口（`src/components/SecretRoom.vue` 中的按钮与弹窗），不包含 ARG 的任何源码、数据或资源。

- 独立仓库：<https://github.com/Brian-Qi/ARG_test>
- 在线访问：<https://briandolph.xyz/arg_01/>
- 部署方式：由 ARG 独立仓库自行构建后，以子目录形式部署到站点的 `/arg_01/`（服务器 `/var/www/briandolph/arg_01/`），与本站点的构建产物互不影响
