...
export default {
  ...
  setup() {
    ...
    // ===== 入场动画 =====
    const introDone = inject('isIntroDone')
    ...
    function loadMusic() {
      ...
      // 延迟下载：仅在用户首次交互后才加载 3.37MB 的 bg.mp3，避免阻塞首屏渲染
...
.main-wrapper {
  ...
  gap: 1.5rem;
  align-items: stretch;
  animation: fade-blur-main-in 0.8s 0.3s ease both;
}
...
.identity {
  ...
  padding: 0.6rem 0;
  margin-bottom: 1rem;
  animation: fade 0.5s 0.15s both;
}
...
.site-footer {
  ...
  color: #fff;
  opacity: 0.7;
  animation: fade 0.5s 0.55s both;
}
