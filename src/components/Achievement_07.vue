  ...
  <div class="achievement-07">
    ...
    <div class="achievement-card">
      ...
      <div class="achievement-reward">
        <div class="reward-badge">
          ...
          <div class="reward-text">
            <p>规则</p>
            <p>就是用来打破的</p>
            <p class="big-slap">好奇心害死猫</p>
            <p class="as-reward">但咱也不是猫</p>
          </div>
          <span class="reward-icon">🔍</span>
        </div>
      </div>
      <div class="achievement-footer">
        ...
        <span class="signature">—— 探索者认证 · 官方认证</span>
      </div>
    </div>
    <div class="button-container">
      <router-link to="/achievements" class="return-button"><span class="button-icon">🏠</span>返回成就</router-link>
    </div>
    <div class="disclaimer"><p>* 好奇心是进步的源泉 *</p><p class="small-text">※ 但别太好奇 ※</p></div>
  </div>
</template>
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      // 不再自动设置 localStorage，防止刷新页面重复触发
    })
    return { currentDate }
  }
}
...
.achievement-07 {
  min-height: 100vh;
  background: linear-gradient(145deg, #2a1a3a 0%, #1a0f2a 100%);
  display: flex;
...
.achievement-07::before {
  content: '';
  position: absolute;
  width: 150%;
  height: 150%;
  background: radial-gradient(circle at center, rgba(155, 89, 182, 0.15) 0%, rgba(142, 68, 173, 0.1) 30%, transparent 70%);
...
.achievement-card {
  ...
  position: relative;
  z-index: 1;
  box-shadow: 0 30px 50px -20px rgba(0,0,0,0.8), 0 0 30px rgba(155, 89, 182, 0.2);
...
.achievement-code { font-size: 0.9rem; color: #9b59b6; background: #2a1a3a; padding: 0.2rem 1rem; border-radius: 40px; font-family: monospace; display: inline-block; margin-bottom: 1rem; border: 1px solid #5a3a7a; }
