  ...
  <div class="guestbook">
    ...
    <template v-else>
      ...
      <div class="message-input-section">
        <div class="message-warning">⚠️ 请友善发言，请勿刷屏、广告或违规内容。每条留言都会公开显示。</div>
        ...
        <div v-if="showSignature" class="signature-input">
          <input v-model="signature" type="text" placeholder="输入你的大名（留空则显示匿名）" maxlength="20" />
        </div>
        ...
        <div class="char-count" :class="{ warn: newMessage.length > 180 }">{{ newMessage.length }}/200</div>
      </div>
...
export default {
  ...
  setup() {
    ...
    const submitNewMessage = async () => {
      ...
      loading.value = true
      try {
        const newMsg = await addMessage(newMessage.value, signature.value.trim())
        if (newMsg && newMsg.id) {
...
<style scoped src="../styles/guestbook.css"></style>
